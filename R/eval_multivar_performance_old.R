...
#'
#' @export
eval_multivar_performance_old  <- function(sim_obj,
                                      fit_obj,
                                      eps = 1e-8,
  ...
  add_summary_block <- function(df, which_effect, label) {
    ...
    data.frame(effect = label, subject = NA_integer_, t(means),
  ...
  if (length(out_list) == 0L) {
    # return an empty but well-formed data.frame
    perf_df <- data.frame(
      effect = character(), subject = integer(),
  ...
  if (isTRUE(reduced.output)) {
    keep <- c("effect","subject","abs_bias","rel_bias","RMSE","sensitivity","specificity","MCC")
