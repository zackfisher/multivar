...
#-------------------------------------------------------#
context("test04: K=1 intercept unpenalized runs without error")
#-------------------------------------------------------#

test_that("K=1 intercept unpenalized model runs successfully", {
  expect_false(is.null(fit))
  expect_false(is.null(fit$mats))
})

test_that("K=1 intercept unpenalized model returns required components", {
...
#-------------------------------------------------------#
context("test04: K=1 common = unique = total")
#-------------------------------------------------------#
...
#-------------------------------------------------------#
context("test04: K=1 no subgroup effects")
#-------------------------------------------------------#
...
#-------------------------------------------------------#
context("test04: K=1 common effects correct")
#-------------------------------------------------------#

expect_equal_to_reference(
  fit$mats$common, "rds/test04_k1_intercept_unpenalized_common.rds"
)

#-------------------------------------------------------#
context("test04: K=1 unique effects correct")
#-------------------------------------------------------#

expect_equal_to_reference(
  fit$mats$unique, "rds/test04_k1_intercept_unpenalized_unique.rds"
)

#-------------------------------------------------------#
context("test04: K=1 total effects correct")
#-------------------------------------------------------#

expect_equal_to_reference(
  fit$mats$total, "rds/test04_k1_intercept_unpenalized_total.rds"
)

#-------------------------------------------------------#
context("test04: K=1 results are deterministic")
#-------------------------------------------------------#

test_that("results are identical to saved snapshots", {
  expect_identical(fit$mats$common, readRDS("rds/test04_k1_intercept_unpenalized_common.rds"))
  expect_identical(fit$mats$unique, readRDS("rds/test04_k1_intercept_unpenalized_unique.rds"))
  expect_identical(fit$mats$total, readRDS("rds/test04_k1_intercept_unpenalized_total.rds"))
})

#-------------------------------------------------------#
context("test04: K=1 evaluation metrics with intercept")
#-------------------------------------------------------#
